...
module.exports = {
    ...
    async execute(interaction) {
        ...
        // 2. THE FIX: Proper Acknowledgment
        if (!interaction.deferred && !interaction.replied) {
            if (interaction.isButton()) {
                await interaction.deferUpdate(); // Acknowledge button click
            } else {
                await interaction.deferReply({ flags: [MessageFlags.Ephemeral] }); // Acknowledge slash command
            }
        }

        const itemsPerPage = 4; // Keeps us within 5 ActionRow limit
        ...
        try {
            ...
            const countRes = await conn.query(
                `SELECT COUNT(*) as total FROM booking_slots WHERE (is_available = FALSE OR is_no_show = TRUE) AND start_time >= NOW() - INTERVAL 3 HOUR`
            );
            const totalPages = Math.ceil(Number(countRes[0].total) / itemsPerPage);

            const rows = await conn.query(
                `SELECT slot_id, start_time, booked_by_name, is_no_show FROM booking_slots 
                 WHERE (is_available = FALSE OR is_no_show = TRUE) AND start_time >= NOW() - INTERVAL 3 HOUR 
            ...
            rows.forEach((row, i) => {
                ...
                // FORCE BOOLEAN: MariaDB returns 0 or 1, Discord needs true or false
                ...
                actionRows.push(new ActionRowBuilder().addComponents(
                    ...
                    new ButtonBuilder()
                        ...
                        .setDisabled(isNoShowBool) // Now correctly receives true/false
                ));
            });
